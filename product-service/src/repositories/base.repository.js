...
// Factory function để tạo base repository functions cho một model
const createBaseRepository = (model) => {
  ...
  const aggregate = async (pipeline) => {
    try {
      ...
      throw new DatabaseError(`Error aggregating ${model.modelName}s: ${error.message}`);
    }
  };

  const updateMany = async (filter, updateData) => {
    try {
      return await model.updateMany(filter, updateData);
    } catch (error) {
      throw new DatabaseError(`Error updating multiple ${model.modelName}s: ${error.message}`);
    }
  };
  ...
  const deleteMany = async (filter) => {
    try {
      ...
      throw new DatabaseError(`Error deleting multiple ${model.modelName}s: ${error.message}`);
    }
  };
